...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval)
{
  ...
  case CAUSE_STORE_PAGE_FAULT:
    ...
    // virtual address that causes the page fault.
    {
      ...
      user_vm_map(current[hartid]->pagetable, ROUNDDOWN(stval, PGSIZE), PGSIZE, (uint64)pa,
...
void smode_trap_handler(void)
{
  ...
  { // soft trap generated by timer interrupt in M mode
    handle_mtimer_trap();
  }
  else if (cause == CAUSE_STORE_PAGE_FAULT || cause == CAUSE_LOAD_PAGE_FAULT || cause == CAUSE_FETCH_PAGE_FAULT)
  {
